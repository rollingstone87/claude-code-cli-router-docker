 ...
 */
async function registerPluginsFromConfig(
    serverInstance: any,
    config: any,
): Promise<void> {
...
async function getServer(options: RunOptions = {}) {
    ...
    if (options.logger !== undefined) {
        ...
        if (config.LOG !== false) {
            ...
            loggerConfig = {
                ...
                // Output to both stdout (for docker logs) and file
                streams: [
    ...
    await Promise.allSettled(
        presets.map(
            async (preset) =>
                await serverInstance.registerNamespace(
                    `/preset/${preset.name}`,
                    preset.config,
                ),
        ),
    ...
    serverInstance.addHook("preHandler", async (req: any, reply: any) => {
        ...
        if (
            ...
            req.pathname !== "/v1/messages"
        ) {
            req.preset = req.pathname
                .replace("/v1/messages", "")
                .replace("/", "");
        }
    });
    ...
    serverInstance.addHook(
        "onSend",
        (req: any, reply: any, payload: any, done: any) => {
            if (req.sessionId && req.pathname.endsWith("/v1/messages")) {
                if (payload instanceof ReadableStream) {
                    if (req.agents) {
                        const abortController = new AbortController();
                        const eventStream = payload.pipeThrough(
                                ...
                                async (data, controller) => {
                                    try {
                                        ...
                                        if (
                                            ...
                                            data.data.index ===
                                                currentToolIndex &&
                                            data.data?.delta?.type ===
                                                "input_json_delta"
                                        ) {
                                            currentToolArgs +=
                                                data.data?.delta?.partial_json;
                                            return undefined;
                                        }
                    ...
                    const read = async (stream: ReadableStream) => {
                        ...
                        try {
                            while (true) {
                                ...
                                if (
                                    !dataStr.startsWith("event: message_delta")
                                ) {
                                    continue;
                                }
                            ...
                            if (
                                ...
                                console.error(
                                    "Background read stream closed prematurely",
                                );
                            } else {
                                console.error(
